...
| 组件 | 选型 | 用途 |
| ---- | ---- | ---- |
| 消息队列 | RabbitMQ | 领域事件异步传递，Exchange + Queue 路由，原生 DLX 支持重试与死信 |
| 限流熔断 | Sentinel | Go 版 Sentinel，令牌桶限流 + 熔断降级 |
| 分布式事务 | Outbox 模式 | 业务事务 + 事件投递的最终一致性，Relay 进程投递到 RabbitMQ |
| 缓存 | Redis 7 | 会话、热点数据、限流计数器 |
| 任务调度 | Asynq | 基于 Redis 的异步任务队列，处理 Embedding 等长耗时任务 |

选 RabbitMQ 而非 Kafka，原因有三：第一，平台事件量为领域事件级（用户注册、课程完成、文档索引等），日均吞吐在百万级以内，RabbitMQ 单机即可承载，无需 Kafka 的分区水平扩展能力；第二，RabbitMQ 原生支持死信交换（DLX）与 TTL 延迟队列，重试与失败兜底无需自建组件，Outbox Relay 投递 + 消费端 confirm/publisher confirm 即可保证可靠投递；第三，RabbitMQ 运维复杂度低于 Kafka，无 ZooKeeper/KRaft 依赖，单镜像即可启动，本地开发与生产环境一致性更高。消息顺序性按「单队列单消费者」保证，跨事件类型按 Exchange 路由键分发，满足领域事件的有序与解耦诉求。
