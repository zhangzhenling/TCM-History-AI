// Package controller hosts the Hertz HTTP handlers for Learning Service.
...
// queryUserID extracts an optional user_id query parameter.
func queryUserID(c *app.RequestContext) int64 {
	raw := string(c.Query("user_id"))
	id, _ := strconv.ParseInt(raw, 10, 64)
	return id
}

// queryLessonID extracts an optional lesson_id query parameter.
func queryLessonID(c *app.RequestContext) int64 {
	raw := string(c.Query("lesson_id"))
	id, _ := strconv.ParseInt(raw, 10, 64)
	return id
}

// queryExamID extracts an optional exam_id query parameter.
func queryExamID(c *app.RequestContext) int64 {
	raw := string(c.Query("exam_id"))
	id, _ := strconv.ParseInt(raw, 10, 64)
	return id
}

// queryCategory extracts an optional category query parameter.
func queryCategory(c *app.RequestContext) string {
	return string(c.Query("category"))
}

// userIDFromHeader extracts the user ID from the X-User-ID header set by
// the API gateway after JWT validation. Returns 0 when absent or invalid.
