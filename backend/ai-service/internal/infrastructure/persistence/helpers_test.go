...
func rewriteDDL(query string) string {
	...
	// Case-insensitive to match GORM's tag output for both `default:now()`
	// and any future variants.
	query = replaceCI(query, "DEFAULT now()", "DEFAULT CURRENT_TIMESTAMP")
	// `timestamptz` (PostgreSQL) → `datetime` (SQLite). SQLite does not
	// understand `timestamptz`; with `_parse_time=true` the mattn driver
	// only parses time strings back into time.Time when the column type is
	// one of {datetime, timestamp, date, time}. Rewriting makes GORM's
	// RETURNING clause round-trip created_at/updated_at cleanly.
	query = replaceCI(query, "timestamptz", "datetime")
	// `jsonb` (PostgreSQL) → `text` (SQLite). SQLite is dynamically typed;
	// the actual storage type is determined per-value. With our wrapped
	// driver returning []byte for strings, json.RawMessage scanning works
