// Package main is the API Gateway entry point. It loads configuration,
...
func main() {
	...
	controller.RegisterRoutes(h, app.ControllerDeps(), app.MiddlewareChain())

	logger.Default().Info("gateway starting",
	...
	waitForShutdown()
	logger.Default().Info("gateway shutting down")
	_ = h.Close()
}
