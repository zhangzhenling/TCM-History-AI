...
//
// Three PostgreSQL-specific adjustments are made for SQLite portability:
//
...
//     `text`, and `'{}'::jsonb` → `'{}'`. This keeps tests portable without
...
//
//  3. GraphNodeRepo.SearchByName uses PostgreSQL's `ILIKE` operator for
//     case-insensitive matching; SQLite has no ILIKE. The ConnPool wrapper
//     rewrites `ILIKE` → `LIKE` in SELECT statements. SQLite's LIKE is
//     case-insensitive for ASCII by default, preserving the method's
//     intended semantics.
...
// DDL and DML fragments to their SQLite equivalents. DDL statements (CREATE /
// ALTER) get full PostgreSQL→SQLite type/default translation; SELECT
// statements get `ILIKE` → `LIKE` so case-insensitive search works.
...
// QueryContext rewrites ILIKE → LIKE in SELECT statements before forwarding.
func (p *sqliteRewritePool) QueryContext(ctx context.Context, query string, args ...interface{}) (*sql.Rows, error) {
	query = rewriteDML(query)
	return p.ConnPool.QueryContext(ctx, query, args...)
}

// QueryRowContext rewrites ILIKE → LIKE in SELECT statements before forwarding.
func (p *sqliteRewritePool) QueryRowContext(ctx context.Context, query string, args ...interface{}) *sql.Row {
	query = rewriteDML(query)
...
// rewriteDML translates PostgreSQL-only DML operators to SQLite-compatible
// equivalents. Currently this only rewrites `ILIKE` → `LIKE` in SELECT
// statements; SQLite's LIKE is case-insensitive for ASCII by default, so the
// case-insensitive matching semantics are preserved.
//
// We restrict to SELECT prefixes so that any user-facing INSERT/UPDATE
// (which might legitimately contain the substring "ilike" in a value) is
// left untouched. Values are passed as placeholders, not inlined, so this is
// safe.
func rewriteDML(query string) string {
	trimmed := strings.TrimSpace(query)
	upper := strings.ToUpper(trimmed)
	if !strings.HasPrefix(upper, "SELECT") {
		return query
	}
	// `ILIKE` (PostgreSQL) → `LIKE` (SQLite). Replace with surrounding
	// whitespace so we don't accidentally match a column named "ilike_x".
	query = replaceCI(query, " ILIKE ", " LIKE ")
	return query
}
