...
服务间通过领域事件实现解耦协作。事件经 RabbitMQ 异步传递：按事件类型建立 topic 类型的 Exchange（如 `tcm.events`），通过 routing key（如 `user.registered`、`course.completed`）路由到各服务绑定的 Queue，单队列内 FIFO 保证消费顺序，跨事件类型互不阻塞。
...
2. 独立的 Relay 进程轮询 Outbox 表，发布事件到 RabbitMQ Exchange（publisher confirm 确认投递成功后标记 `sent`）。
3. 消费方幂等处理事件，失败经 TTL 延迟队列重试，超过阈值通过 DLX 进入死信队列。
