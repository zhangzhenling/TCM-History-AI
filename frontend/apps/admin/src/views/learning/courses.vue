...
// 课程管理列表：调用 apis.learning.listCourses()，表格展示 + 新增/编辑（Modal 占位）。
...
const columns = [
  ...
  { title: '课程名', dataIndex: 'title', key: 'title' },
  { title: '分类', dataIndex: 'category', key: 'category', width: 120 },
  { title: '难度', dataIndex: 'difficulty', key: 'difficulty', width: 100 },
  { title: '课时数', dataIndex: 'lesson_count', key: 'lesson_count', width: 90 },
...
async function load() {
  loading.value = true;
  try {
    const res = await apis.learning.listCourses({
      page: query.page,
...
<template>
  <div class="admin-page">
    ...
    <div class="table-card">
      <div class="toolbar">
        <Button type="primary" @click="openModal">新增课程</Button>
      </div>
      <Table
      ...
      >
        <template #bodyCell="{ text, column }">
          <template v-if="column.dataIndex === 'difficulty'">
            <Tag :color="difficultyColorMap[text] || 'default'">{{ text || '—' }}</Tag>
          </template>
          <template v-else-if="column.dataIndex === 'is_published'">
    ...
    >
      <p class="modal-placeholder">表单占位：后续接入完整的课程新增 / 编辑表单。</p>
    </Modal>
  </div>
</template>
